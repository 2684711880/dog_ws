...
class DM_Serial:
    def __init__(self, port: str, baudrate: int):
        self.port = port                        # 串口名
        self.baudrate = int(baudrate)           # 串口波特率
        self.timeout = 0.0                      # 非阻塞
        self.ser: Optional[serial.Serial] = None    
        self._buf = bytearray()                 # 缓冲区

        # 统计
        self.cnt_ok = 0                         # 成功帧数
        self.cnt_crc = 0                        # CRC 失败
        self.cnt_short = 0                      # 短桢
        self.cnt_nohdr = 0                      # 无帧头

        # 后台读线程状态
        self._th: Optional[threading.Thread] = None         # 线程对象
        self._stop_evt: Optional[threading.Event] = None    # 停止事件
        self._read_sleep = 0.001                            # 读线程小睡控制 CPU

        # 最新数据（线程安全）
        self._latest_lock = threading.Lock()                # 锁保护
        self._latest_pkt: Optional[Tuple[int, Tuple[float, float, float]]] = None   # 最新数据锁
        self._latest_ts: float = 0.0                        # 最新数据时间戳
        self._latest_count: int = 0                         # 总更新记数
        # 按 RID 分别缓存最新值，避免“只拿最后一帧”导致中间帧信息丢失
        self._latest_by_rid: Dict[int, Tuple[float, float, float]] = {}
        self._latest_ts_by_rid: Dict[int, float] = {}
        self._latest_count_by_rid: Dict[int, int] = {}
    ...
    def get_latest_by_rid(self, rid: int) -> Tuple[Optional[Tuple[float, float, float]], float, int]:
        """
        线程安全地获取指定 RID 的最新值（values, timestamp, count）。
        values 为 (x, y, z)，count 为该 RID 更新计数（用于去重）。
        rid：RID值（有效为0x01,0x02,0x03）
        """
        with self._latest_lock:
            values = self._latest_by_rid.get(rid)
            ts = self._latest_ts_by_rid.get(rid, 0.0)
            count = self._latest_count_by_rid.get(rid, 0)
            return values, ts, count
                ...
                if frames:
                    with self._latest_lock:
                        ...
                        for pkt in frames:
                            ...
                            self._latest_by_rid[rid] = values
                            self._latest_ts_by_rid[rid] = now_ts
                            self._latest_count_by_rid[rid] = self._latest_count_by_rid.get(rid, 0) + 1
